...
# Function to handle selection and process data
def on_select(event):
...
def filter_data():
    ...
    if original_data_all:
        ...
        print(result)  # You can replace this with your desired output handling
        return result
...
def create_gui():
    ...
    # 按鈕：篩選
    filter_button = ttk.Button(root, text="查詢", command=filter_data)
    filter_button.pack()

    # 按鈕：匯出 CSV
    export_button = ttk.Button(root, text="匯出 CSV", command=export_to_csv)
    export_button.pack()
